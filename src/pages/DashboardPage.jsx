      ...
      >
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-soft text-sm">
            {new Date(project.created_at).toLocaleDateString('en-IN', { year: 'numeric', month: 'long', day: 'numeric' })}
          </p>
          {project.description && (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          {project.exterior_image && (
            <div className="rounded-2xl overflow-hidden border border-white/10 relative bg-black/50">
          ...
          {project.floorplan_image && (
            <div className="rounded-2xl border border-white/10 bg-black/50 md:col-span-2">
              ...
              <img src={project.floorplan_image} alt="Floorplan" className="rounded-b-2xl w-full h-auto" />
            </div>
          )}
          {!project.exterior_image && !project.interior_image && !project.floorplan_image && (
            <div className="md:col-span-2 text-center py-12 text-gray-soft">
              ...
              <p>Images may have expired. Generate a new design to see full previews.</p>
            </div>
          )}
        </div>
        ...
        <div className="flex gap-4 justify-end flex-wrap">
          {project.pdf_report && (
            <a
              href={`${backendBase}/download/${project.pdf_report}`}
              target="_blank"
              rel="noreferrer"
  ...
  return (
    <div className="pt-32 px-6 max-w-7xl mx-auto min-h-screen pb-20">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-10">
        ...
        <Link
          to="/studio"
          className="bg-gold text-primary px-6 py-3 rounded-full font-semibold flex items-center gap-2 hover:bg-gold-light transition-all shadow-[0_0_20px_rgba(212,175,55,0.3)] w-full sm:w-auto justify-center"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <StatCard title="Total Generations" value={projects.length} icon={ImageIcon} />
        <StatCard title="Saved on Device" value={projects.length} icon={Activity} />
      </div>
        ...
        <div className="glass-panel p-8 rounded-3xl border border-gold/30 bg-gradient-to-r from-gold/10 via-transparent to-transparent flex flex-col md:flex-row items-start md:items-center justify-between gap-6 relative overflow-hidden group">
          <div className="z-10 flex-1">
            ...
            <p className="text-sm text-gray-soft max-w-2xl leading-relaxed">
              Have questions about interior design, material selection, structural layouts, or budgets? Get instant expert guidance from your personal AI architect.
            </p>
          </div>
      ...
      {projects.length === 0 ? (
        <div className="text-center py-20 bg-primary-light/30 rounded-3xl border border-white/5">
          ...
          <Link to="/studio" className="bg-gold text-primary px-6 py-3 rounded-full font-semibold hover:bg-gold-light transition-all">
            Start Designing
          </Link>
        </div>
      ) : (
